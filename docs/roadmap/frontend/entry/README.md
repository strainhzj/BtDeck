...
| 双语 i18n | `i18n/` | 双语基础设施（桌面双语 P1）：vue-i18n@8.28.2 单例 + zh-CN/en 消息树（navigation/time/el）+ 语言解析（手动偏好>浏览器语言>默认中文，localStorage `btdeck-lang`）+ 缺译回退 + `translate/translateChoice/routeTitle/resolvePageTitle` 同源入口；选型与风险见 PLANS/bilingual/p1-i18n-decision.md |
