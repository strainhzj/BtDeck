...
| `utils/torrentBatch.ts` | 批量操作纯函数集合（可单测）；✨2026-08-20 展示对齐判定新增共享 helper：`hasTrackerError` L656、`showTrackerErrorTag` L677（error 状态不打标）、`getTorrentErrorReason` L687（errorReason → tracker 消息 → 兜底回退链，两视图委托调用）；✨2026-08-27 新增 `countMatchedTrackerRows` L666（统计含 tracker 域名筛选命中标记的行数，供两视图 `[tracker-filter]` 观察日志）；✨2026-08-29 新增 `buildSpeedSnapshot` L545 的 200/206 增量合并与终态归一、`collectRuntimeStateReconcileCandidates` L460 的复合键连续未命中候选收敛 |
