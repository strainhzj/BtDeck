...
| `../mobile/notifications.vue` | 移动通知列表：无限滚动分页追加（按 id 去重防跨页重复）+ 30s 静默刷新（已翻页只同步角标）；✨2026-09-05 无限滚动改 `mixins/window-infinite-scroll.ts`（与种子页同源失控根修） |
