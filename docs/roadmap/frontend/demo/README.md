...
| 脱敏 fixture | `src/demo/fixtures/index.ts`（790 行；`DEMO_FIXTURE_BUNDLE` L770） | 固定演示用户、下载器、种子、Tracker、任务、日志、回收站、孤儿文件和模板数据；主机使用 `.example.invalid`，路径使用 `/demo/*`；✨2026-09-22 W5：DEMO_MCP_CATALOG 六项补 descriptionEn（镜像后端成对文案）+ DEMO_MCP_API_KEY 固定演示密钥（`btdmcp_`+43 位格式对齐，不进真实认证链） |
| fixture 说明 | `src/demo/fixtures/README.md` | 字段分组、安全边界、重置和演示脚本说明 |
| 内存状态仓库 | `src/demo/demo-store.ts`（802 行；`DemoStore` L176、singleton L802） | 提供分页/筛选、有限 CRUD、种子状态、通知、模板、任务、关键词池和孤儿文件本地状态突变，禁止持久化真实业务数据；✨2026-09-22 W5：getMcpApiKey/rotateMcpApiKey（demo 密钥 revision CAS，与真实端点同语义） |
| 请求分流 | `src/demo/demo-request.ts`（1002 行；`handleDemoRequest` L348、`demoRequest` L992） | 按 method + path 集中返回 API 信封、Blob、降级响应和特殊页面契约，不发真实网络；✨2026-09-22 W5：`/mcp/apikey` GET 与 `/mcp/apikey/rotate` POST 分支（409 语义与真实端点一致） |
