...
| 关键词 | 文件 | 一句话职责 |
|--------|------|-----------|
| 种子管理核心 torrent | `torrents.ts` | 🔵 种子管理核心：列表/添加/批量/异步删除/重复检测/文件备份等；`TorrentListParams.tracker_domain` L192 支持 Tracker 主机域名多选，`single_error_only` L196 支持错误单种全局唯一排查，`getTrackerDomains()` L224 读取定时同步已采集的域名；均复用 `getTorrentList()` 的筛选、排序与分页契约 |
