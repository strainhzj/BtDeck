...
| 种子同步 torrent-sync | `torrent_sync.py` | 种子同步端点 + 同步辅助函数；v1.0.6.25 起同步写入用 `torrent_ratio_values` |
| 种子聚合 torrents | `torrents.py` | 种子聚合路由器（include_router 合并 6 个子路由） |
| 异步种子 DB torrents-async | `torrents_async.py` | 异步版种子 DB 操作（供定时任务用）；v1.0.6.25 起写入经 `torrent_ratio_values` |
