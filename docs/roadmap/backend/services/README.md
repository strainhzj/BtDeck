...
| `advanced_search.py` | 1180 | 3 class, 0 def | 高级搜索服务（ORM 查询引擎，13 字段搜索，预留 FTS5） |
...
| `torrent_crud_service.py` | 650 | 0 class, 26 def | 种子 DB CRUD 服务（模块级函数集合，无类） |
...
- 本分支第三层待后续会话按模式 B 补齐（建议优先级：`torrent_deletion_by_level.py` 1670 行、`advanced_search.py` 1180 行、`orphan_file_service.py` 1088 行）
