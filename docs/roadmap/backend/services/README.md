...
### services/ 根（55 个文件，不计 `__init__.py`；✨2026-09-09 计数校准：含本批 moviepilot 两服务与 mcp_settings/torrent_add_helpers 等历史漂移；✨2026-09-22 W5 +mcp_apikey_service 实测校准）
...
| 孤儿统计缓存 orphan-stats | `orphan_stats_cache.py` | 孤儿列表 scan_context 全量统计（remaining/ignored）进程内缓存：按 display_scan.scan_id 键控、写咽喉点失效（epoch 代际防失效后回写），单进程部署无跨进程一致性问题 |
...
### downloader_adapters/ 子包（5 个文件 + `__init__.py`）
...
### tag_adapters/ 子包（5 个文件 + `__init__.py`）
...
- **模块级函数 vs 类**：`torrent_crud_service.py`（26 个函数无类）与 `torrent_metadata.py`（21 个函数无类）是有意写成工具函数集合，与 `DashboardService` 等类风格并存
...
- 已完成：[orphan_file_service.md](./orphan_file_service.md)（3902 行，2026-08-16 第二批实测）；其余建议优先级：`torrent_deletion_by_level.py`、`advanced_search.py`（1471 行）
