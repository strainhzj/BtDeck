...
| `orphan_purge_job_service.py` | 403 | 2 class, 1 def | 隔离区彻底删除持久化任务：原子领取/串行后台执行/重启恢复/通知幂等补偿 |
| `orphan_quarantine.py` | 313 | 0 class, 11 def | 孤儿隔离区管理（移入 → 保留期 → 物理删除），仅用 `os.rmdir` 回收记录根内空目录 |
