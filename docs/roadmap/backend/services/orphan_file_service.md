...
| 行数 | 3438（实测 PowerShell `Get-Content`） |
...
- **安全清理**：预览、前缀预览、手动和定时自动清理共用最新 completed/scan_id 门禁；超 50000 条且未核查路径映射+孤儿样本时拒绝，未复核门禁会向仍有活跃候选的后续小扫描传递；删除前实时复核文件身份，不提供 force。
...
| L384 | `_evaluate_cleanup_snapshot` | def | completed/scan_id/超量复核共用清理门禁 |
| L440 | `_build_realtime_manifest` | async def | 构建删除前实时 manifest |
| L650 | `get_latest_scan_result` | async def | 最新扫描结果 |
| L686 | `get_hardlink_copy_locations` | async def | **批量按需定位配置目录内副本** |
| L774 | `get_orphan_list` | async def | **稳定当前明细分页 + 扫描上下文** |
| L915 | `get_orphan_list_grouped` | async def | 文件夹父页 SQL 聚合，不加载/不 stat 全部子项 |
| L1149 | `get_orphan_folder_children` | async def | **展开后子项独立分页，仅可见页统计硬链接** |
| L1220 | `_enrich_hardlink_copy_counts` | static | 在线补充 `st_nlink - 1`，不可访问为 `None` |
| L1290 | `reconcile_stable_candidate_details` | async def | keyset 分批对账稳定隔离候选明细，每页统一进入 `db_write_scope` |
| L1448 | `prefix_match_preview` | async def | 路径前缀预览（共用清理门禁） |
| L1489 | `cleanup_preview` | async def | 清理预览 |
| L1535 | `cleanup_orphans` | async def | **后台手动清理任务执行** |
| L1809 | `set_ignored` | async def | 设置忽视 |
| L1976 | `auto_cleanup_expired` | async def | 过期自动清理（共用超量门禁） |
| L2173 | `purge_expired_quarantine` | async def | 过期隔离清除 |
| L2366 | `get_quarantine_list` | async def | 隔离区列表 |
| L2483 | `restore_quarantined` | async def | 恢复隔离 |
| L2693 | `purge_quarantine_now` | async def | 后台立即彻底删除 |
| L2844 | `_purge_single_candidate` | async def | 单个彻底删除 |
| L2968 | `_detect_hardlink_copies` | async def | 删除前枚举硬链接副本 |
| L3232 | `_recover_interrupted_operations` | async def | 中断操作恢复 |
...
- **定位**：`orphan_file_service.py:1149`
...
- **定位**：`orphan_file_service.py:1475`
- **职责**：清理门禁（最新 completed + scan_id + 超量双复核 + 实时身份复核）后安全隔离 + 标记 + 审计；不提供 force 绕过。
- **前置**：`_check_cleanup_allowed`（L427）、`_build_realtime_manifest`（L440）；worker 读取已占用 ID 时不启用查询排除。
