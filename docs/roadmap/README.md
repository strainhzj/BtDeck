...
| 最近同步 | 2026-09-02：前端静态展示 Demo 新增 `src/demo/`、Demo 模式指示器、构建入口与独立 Nginx 静态服务；行号已按当前源码复测 |
