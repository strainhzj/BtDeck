...
│   ├── services/        业务服务层 + 下载器/标签适配器
│   ├── core/            基础设施（config / path_mapping / file_ops …）+ ⚠ 含 5 个孤儿文件
...
│   └── infra/           utils + startup + migrations + alembic
...
│   ├── components-layout/  通用组件 + 布局骨架
│   └── utils-types/     工具 / 类型 / 常量 / 指令
├── deploy/           ← 多部署模式（Docker / PyInstaller / Inno Setup / fpm）
├── tests/            ← 测试（backend pytest 96 个 test_*.py + frontend jest unit）
...
| ↳ core | 基础设施（config/path_mapping/file_ops/tracker_*），⚠ 含 5 个 0 引用孤儿文件 | [backend/core/README.md](./backend/core/README.md) |
...
| ↳ components-layout | 通用组件（Pagination/Breadcrumb/ThemeSwitcher…）+ layout 骨架 | [frontend/components-layout/README.md](./frontend/components-layout/README.md) |
...
| **tests** | 后端 pytest（96 个 test_*.py，按子目录组织）+ 前端 jest unit | [tests/README.md](./tests/README.md) |
