...
- 修复：新增 _apply_sqlite_pragmas 事件回调，挂到 engine 和 async_engine.sync_engine 的
  connect 事件，强制下发 journal_mode=WAL / synchronous=NORMAL / busy_timeout=15000。
...
class TestSqlitePragmasRegression:
    ...
    def test_apply_sqlite_pragmas_sets_busy_timeout(self, tmp_path):
        """_apply_sqlite_pragmas 必须对连接下发 busy_timeout=15000 / journal_mode=wal / synchronous=normal。
        ...
        try:
            ...
            # busy_timeout 必须 15000ms（15s），缓解多任务并发写的 locked
            # （主修复后锁竞争窗口已极短：db_write_scope 串行化写者，二级兜底 15s 对齐前端 axios timeout=20s）
            bt = conn.execute("PRAGMA busy_timeout").fetchone()[0]
            assert bt == 15000, (
                f"busy_timeout 必须为 15000ms（实际 {bt}）：" "缺此项是多任务并发写时 database is locked 的根因"
            )
            ...
            assert sync in ("1", "normal"), (
                f"synchronous 必须为 normal（实际 {sync}）：" "默认 FULL 会每笔事务 fsync，放大锁占用时间"
            )
        finally:
            conn.close()
    ...
    def test_event_listener_registered_for_connect(self):
        ...
        assert event.contains(engine, "connect", _apply_sqlite_pragmas), (
            "engine 必须注册 _apply_sqlite_pragmas 到 connect 事件：" "同步引擎的每个新连接需要强制下发 PRAGMA"
        )
