...
class TestMigrationChainIntegrity:
    ...
    def test_empty_db_upgrade_head_builds_full_schema(self, tmp_path):
        """空库 alembic upgrade head 应建起完整 schema（40 张业务表）。
        ...
        assert (
            count == 40
        ), f"空库 upgrade 应建 40 张业务表（含 orphan_purge_job + sync_checkpoints + 副本预扫描 + refresh_tokens + moviepilot 集成两表 + RSS 订阅五表），实际 {count}"
...
class TestDatabasePathRouting:
    ...
    def test_upgrade_writes_to_specified_database_path(self, tmp_path):
        ...
        assert _table_count(str(target_db)) == 40
