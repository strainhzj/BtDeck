...
async def get_task_logs_endpoint(
    ...
    try:
        # 获取任务日志
        result = await get_task_logs(task_name=task_name, success=success, limit=limit, offset=offset)

        return CommonResponse(status="success", msg="获取任务日志成功", code="200", data=result)
...
async def get_task_statistics_endpoint(_user=Depends(require_authenticated_user)):
    ...
    try:
        ...
        return CommonResponse(status="success", msg="获取任务统计成功", code="200", data=stats)

    except Exception as e:
        return CommonResponse(status="error", msg=f"获取任务统计失败: {str(e)}", code="500", data=None)
