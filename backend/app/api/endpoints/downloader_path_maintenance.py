...
@router.get(
    "/{downloader_id}/paths", summary="获取下载器路径列表", response_model=CommonResponse, tags=["下载器路径维护"]
)
def get_paths(
    ...
    try:
        ...
        paths = service.get_paths_by_downloader(
            downloader_id=downloader_id, path_type=path_type, is_enabled=is_enabled  # 直接使用字符串，无需转换
        )
        ...
        # 7. 构建符合前端期望的响应格式
        response_data = {"downloader_id": downloader_id, "downloader_name": downloader_name, "paths": path_list}

        return CommonResponse(status="success", msg="获取路径列表成功", code="200", data=response_data)
...
@router.post("/{downloader_id}/paths", summary="添加下载器路径", response_model=CommonResponse, tags=["下载器路径维护"])
def create_path(
    downloader_id: Annotated[str, Path(description="下载器ID")],
...
@router.put(
    "/{downloader_id}/paths/{path_id}", summary="更新下载器路径", response_model=CommonResponse, tags=["下载器路径维护"]
)
def update_path(
    downloader_id: Annotated[str, Path(description="下载器ID")],
    ...
    try:
        ...
        success = service.update_path(
            path_id=path_id, path_value=request_data.path_value, is_enabled=request_data.is_enabled
        )
...
@router.delete(
    "/{downloader_id}/paths/{path_id}", summary="删除下载器路径", response_model=CommonResponse, tags=["下载器路径维护"]
)
def delete_path(
    downloader_id: Annotated[str, Path(description="下载器ID")],
    path_id: Annotated[int, Path(description="路径ID")],
    ...
    try:
        ...
        if not success:
            return CommonResponse(status="error", msg="路径不存在或删除失败", code="404", data=None)

        return CommonResponse(status="success", msg="删除路径成功", code="200", data=None)
...
def get_path_statistics(
    ...
    try:
        ...
        return CommonResponse(status="success", msg="获取路径统计成功", code="200", data=stats)
