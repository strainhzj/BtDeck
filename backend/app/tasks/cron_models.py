...
class CronTask(Base):
    ...
    last_attempt_at = Column(DateTime, nullable=True, comment="最近一次执行尝试时间（所有执行更新）")
    last_outcome = Column(
        String(20),
        nullable=True,
        comment="最近一次业务结果（六态：success/partial/skipped/failed/no_action/cancelled）",
    )
    last_skip_reason = Column(
        String(50),
        nullable=True,
        comment="最近一次跳过原因机器码（resource_busy/already_running/outside_budget/downloader_offline）",
    )
    last_run_id = Column(String(64), nullable=True, comment="最近一次运行 ID（日志溯源）")
