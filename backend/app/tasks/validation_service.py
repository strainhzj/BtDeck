...
class ScriptValidationService:
    ...
    def __init__(self):
        self.script_type_map = {0: "shell", 1: "batch", 2: "powershell", 3: "python"}
    ...
    async def validate_script(self, content: str, script_type: int) -> ScriptValidationResponse:
        ...
        try:
            ...
            if script_type == 0:  # Shell脚本
                ...
                return ScriptValidationResponse(valid=False, errors=[], message=f"不支持的脚本类型: {script_type}")

        except Exception as e:
            logger.error(f"脚本语法校验失败: {str(e)}")
            return ScriptValidationResponse(valid=False, errors=[], message=f"校验过程中发生错误: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                result = await asyncio.create_subprocess_exec(
                    "bash", "-n", temp_file, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
                )
            ...
            return ScriptValidationResponse(valid=False, errors=[], message=f"Shell脚本校验失败: {str(e)}")
    ...
    async def _validate_batch_script(self, content: str) -> ScriptValidationResponse:
        ...
        try:
            ...
            return ScriptValidationResponse(valid=False, errors=[], message=f"CMD脚本校验失败: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                result = await asyncio.create_subprocess_exec(
                    ...
                    "-NoProfile",
                    "-Command",
                    f"Get-Command -Syntax (Get-Content '{temp_file}' -Raw)",
    ...
    async def _validate_python_script(self, content: str) -> ScriptValidationResponse:
        ...
        try:
            ...
            return ScriptValidationResponse(valid=False, errors=errors, message="Python语法错误")

        except Exception as e:
            return ScriptValidationResponse(valid=False, errors=[], message=f"Python脚本校验失败: {str(e)}")
    ...
    async def validate_cron_expression(self, expression: str) -> CronValidationResponse:
        ...
        try:
            ...
            if not expression or not expression.strip():
                return CronValidationResponse(valid=False, message="Cron表达式不能为空")

            parts = expression.strip().split()
            if len(parts) != 5:
                return CronValidationResponse(valid=False, message="Cron表达式必须包含5个字段: 分 时 日 月 周")
            ...
            try:
                ...
                return CronValidationResponse(
                    valid=True, message="Cron表达式有效", description=description, executionTimes=execution_time_info
                )

            except ValueError as e:
                return CronValidationResponse(valid=False, message=f"无效的Cron表达式: {str(e)}")

        except Exception as e:
            logger.error(f"Cron表达式校验失败: {str(e)}")
            return CronValidationResponse(valid=False, message=f"校验过程中发生错误: {str(e)}")
    ...
    async def validate_class_path(self, class_path: str) -> PythonClassValidationResponse:
        ...
        try:
            ...
            if not class_path or not class_path.strip():
                return PythonClassValidationResponse(valid=False, exists=False, message="类路径不能为空")

            # 检查格式
            if "." not in class_path:
                return PythonClassValidationResponse(
                    valid=False, exists=False, message="类路径格式不正确，应为 module.submodule.ClassName"
                )

            parts = class_path.split(".")
            if len(parts) < 2:
                return PythonClassValidationResponse(valid=False, exists=False, message="类路径格式不正确")
            ...
            try:
                ...
                if hasattr(module, class_name):
                    ...
                    return PythonClassValidationResponse(
                        valid=True, exists=True, classInfo=class_info, message="类路径验证成功"
                    )
                else:
                    return PythonClassValidationResponse(
                        valid=False, exists=False, message=f"模块 {module_path} 中不存在类 {class_name}"
                    )

            except ImportError as e:
                return PythonClassValidationResponse(
                    valid=False, exists=False, message=f"无法导入模块 {module_path}: {str(e)}"
                )

        except Exception as e:
            logger.error(f"Python类路径验证失败: {str(e)}")
            return PythonClassValidationResponse(valid=False, exists=False, message=f"验证过程中发生错误: {str(e)}")
