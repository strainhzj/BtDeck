...
class DownloaderPathScanTask:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                for downloader in downloaders:
                    try:
                        # 扫描单个下载器的路径
                        scan_result = await self._scan_downloader_paths(db, downloader)
                        ...
                        logger.error(f"扫描下载器 {downloader.nickname} 时出错: {str(e)}", exc_info=True)
                        details.append(
                            {
                                "downloader_id": downloader.downloader_id,
                                "downloader_name": downloader.nickname,
                ...
                # 记录任务日志到 task_logs 表
                await self._log_task_execution(db, start_time, datetime.now(), True, result)
            ...
            try:
                async with AsyncSessionLocal() as db:
                    await self._log_task_execution(db, start_time, datetime.now(), False, error_result)
            except Exception as log_error:
                logger.error(f"记录任务日志失败: {str(log_error)}")

            return error_result

    async def _scan_downloader_paths(self, db: AsyncSession, downloader: BtDownloaders) -> Dict[str, Any]:
        """
        扫描单个下载器的路径
        ...
        torrent_result = await db.execute(
            select(TorrentInfo.save_path)
            .where(TorrentInfo.downloader_id == downloader.downloader_id, TorrentInfo.dr == 0)  # 只查询未删除的记录
            .distinct()
        )
        ...
        if downloader.path_mapping:
            try:
                ...
                logger.warning(f"下载器 {downloader.nickname} 的路径映射配置解析失败: {str(e)}")
        ...
        if downloader.path_mapping_rules and existing_mappings:
            updated_external_count = await self._update_external_paths(db, downloader, existing_mappings)

        # 6. 如果有新路径，更新 path_mapping
        if new_paths:
            await self._update_path_mapping(db, downloader, new_paths, existing_mappings, default_mapping)
    ...
    async def _update_external_paths(
        self, db: AsyncSession, downloader: BtDownloaders, existing_mappings: List[Dict]
    ) -> int:
        """
        更新现有映射的 external 路径
        ...
        try:
            ...
            if updated_count > 0:
                ...
                new_config = {
                    ...
                    "discovery_stats": (
                        mapping_service.mappings[0].get("discovery_stats", {}) if mapping_service.mappings else {}
                    ),
    ...
    async def _log_task_execution(
        self, db: AsyncSession, start_time: datetime, end_time: datetime, success: bool, result: Dict[str, Any]
    ):
        """
        ...
        try:
            ...
            # 构建日志详情
            log_detail = json.dumps(result, ensure_ascii=False, default=str)[:2000]  # 限制长度
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "average_scan_per_execution": (self.total_scan_count / self.execution_count),
            "average_new_paths_per_execution": (self.total_new_paths / self.execution_count),
        ...
        try:
            if not default_path and normalized_paths:
                ...
                path_count_result = await db.execute(
                    select(TorrentInfo.save_path, func.count(TorrentInfo.hash).label("count"))
                    .where(TorrentInfo.downloader_id == downloader.downloader_id, TorrentInfo.dr == 0)
                    .group_by(TorrentInfo.save_path)
                    .order_by(func.count(TorrentInfo.hash).desc())
                    .limit(1)
                )
                first_path = path_count_result.first()
                if first_path:
                    default_path = first_path[0]
                    logger.info(f"下载器 {downloader.nickname} 使用统计得出的默认路径: {default_path}")
            ...
            for path_value, torrent_count in path_counts.items():
                await self._sync_active_path(db, downloader.downloader_id, path_value, torrent_count)

            # 5. 清理不再使用的路径（可选，标记为禁用）
            await self._cleanup_obsolete_paths(db, downloader.downloader_id, set(path_counts.keys()))

            logger.info(
                f"下载器 {downloader.nickname} 路径同步完成: " f"默认路径={default_path}, 在用路径={len(path_counts)}个"
            )

        except Exception as e:
            logger.error(f"同步下载器 {downloader.nickname} 到 maintenance 表失败: {str(e)}", exc_info=True)

    async def _get_default_path_from_downloader(self, downloader: BtDownloaders) -> Optional[str]:
        """
        从下载器客户端获取默认保存路径
        ...
        try:
            ...
            if downloader_vo.downloader_type == 0:  # qBittorrent
                ...
                try:
                    ...
                    # 替代为 get_session()，返回 Session 对象。
                    #
                    # 字段访问说明：不能用 getattr(session, "download_dir", None) 兜底——
                    # Session.download_dir 是 @property，内部 return self.fields["download-dir"]，
                    # 字段缺失时抛 KeyError 而非 AttributeError，getattr 的 default 不捕获 KeyError。
                    # 因此直接走 session.fields.get("download-dir") 显式取值，缺失返回 None。
                    session = await asyncio.to_thread(client.get_session)
                    default_path = session.fields.get("download-dir") if session else None
                    if default_path:
        ...
        try:
            ...
            if existing_record:
                ...
                if existing_record.path_value != default_path:
                    ...
                    logger.debug(f"更新下载器 {downloader_id} 的默认路径: {default_path}")
                else:
                    # 仅更新时间戳
                    existing_record.last_updated_time = datetime.utcnow()
            else:
