...
class RequestDownloader(BaseModel):
    ...
    port: int | None = Field(None, description="端口", examples=[1])
...
class ListDownloader(BaseModel):
    downloader_id: str | None = Field(None, description="下载器数据id，用于精确查询", examples=["1"])
    host: str | None = Field(None, description="下载器主机地址", examples=["1.1.1.1"])
    nickname: str | None = Field(None, description="下载器别名", examples=["qb"])
    is_search: str | None = Field(None, description="是否启用搜索的下载器标识，0表示停用，1表示启用", examples=["1"])
    downloader_type: int | None = Field(None, description="下载器类型(0=qBittorrent, 1=Transmission)", examples=[0])
    enabled: str | None = Field(None, description="下载器启用标识，0表示停用，1表示启用", examples=["1"])
...
class UpdateDownloader(BaseModel):
    ...
    password: str | None = Field(None, description="新密码（不修改请留空）", examples=[""])
    old_password: str | None = Field(None, description="原密码（修改密码或用户名时必填）", examples=[""])
    is_search: bool = Field(description="是否启用搜索的下载器标识，0表示停用，1表示启用", examples=[True])
    enabled: bool = Field(description="下载器启用标识，0表示停用，1表示启用", examples=[True])
    downloader_type: int | None = Field(None, description="下载器类型(0=qBittorrent, 1=Transmission)", examples=[0])
    port: int | None = Field(None, description="端口", examples=[1])
...
class DownloaderCheckVO(BaseModel):
    ...
    fail_time: int = Field(0, description="校验失败次数", examples=[0])
    # 添加额外的字段以供同步任务使用
    downloader_id: str | None = Field(None, description="下载器ID", examples=["1"])
    host: str | None = Field(None, description="下载器主机", examples=["127.0.0.1"])
    port: int | None = Field(None, description="下载器端口", examples=[8080])
    username: str | None = Field(None, description="下载器用户名", examples=["admin"])
    password: str | None = Field(None, description="下载器密码", examples=["admin123"])
    downloader_type: int | None = Field(None, description="下载器类型(0=qBittorrent, 1=Transmission)", examples=[0])
    torrent_save_path: Optional[str] = Field(None, description="torrent_save_path")

    # 实时状态字段（缓存使用）
    upload_speed: float | None = Field(None, description="上传速度(KB/s)", examples=[1024.5])
    download_speed: float | None = Field(None, description="下载速度(KB/s)", examples=[2048.7])
    downloading_count: int | None = Field(None, description="下载中种子数量", examples=[5])
    seeding_count: int | None = Field(None, description="做种中种子数量", examples=[10])
