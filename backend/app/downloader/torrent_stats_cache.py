...
class TorrentStatsCache:
    ...
    def __init__(self, downloader_id: str, full_sync_interval: int = 3600, cache_ttl: int = 7200):
        """初始化缓存管理器
        ...
        logger.debug(
            f"初始化种子缓存: downloader_id={downloader_id}, " f"全量间隔={full_sync_interval}s, TTL={cache_ttl}s"
        )
    ...
    def update_cache(self, torrents: List[Dict[str, Any]]):
        ...
        for torrent in torrents:
            ...
            # 更新或添加条目
            self.cache[hash_str] = TorrentCacheEntry(status=status, last_seen=time.time(), name=name)
            updated_count += 1
