...
class OrphanFileService:
    ...
    async def _check_cleanup_allowed(
    ...
    async def _build_realtime_manifest(
        ...
        """
        try:
            return await TorrentManifestBuilder(store).build(
                required_downloader_ids=required_downloader_ids
            )
    ...
    def _path_authorized(
        candidate: OrphanCurrentCandidate, manifest: ManifestSnapshot
    ) -> bool:
        ...
        for root, downloader_id in manifest.scan_roots:
            ...
            try:
                if os.path.commonpath(
                    [candidate_path, os.path.realpath(root)]
    ...
    async def get_orphan_list(
        ...
        # 只展示最新 completed 批次，避免 UI 选择历史孤儿记录。
        conditions = [OrphanFile.scan_id == latest.scan_id]
    ...
    async def cleanup_preview(
        self, orphan_ids: List[int], scan_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """清理预览（返回文件数 + 总大小）。
    ...
    async def cleanup_orphans(
        ...
        if not _lease_acquired:
            ...
            try:
                ...
                return {
                    ...
                    "failed_list": [
                        {"id": oid, "reason": str(exc)} for oid in orphan_ids
                    ],
                    "rejected": True,
                    "error": str(exc),
                    "total_size": 0,
        ...
        if not gate["allowed"]:
            return {
                ...
                "failed_count": len(orphan_ids),
                "failed_list": [
                    {"id": oid, "reason": gate["reason"]} for oid in orphan_ids
                ],
                "rejected": True,
                "error": gate["reason"],
                "total_size": 0,
        ...
        candidate_result = await self.db.execute(
            select(OrphanCurrentCandidate).where(
                OrphanCurrentCandidate.canonical_path.in_(
                    [normalize_path(item.file_path) for item in items]
                ),
        ...
        if manifest is None:
            reason = (
                "实时 manifest 构建失败，无法确认文件是否仍被种子引用（fail-closed）"
            )
            ...
            return {
                ...
                "failed_list": [
                    {"id": i.id, "file_path": i.file_path, "reason": reason}
                    for i in items
                ],
                "total_size": 0,
            }
        await self._recover_interrupted_operations(manifest, store=store, lease_handle=_lease_handle)
        ...
        for item in items:
            try:
                ...
                if not ok:
                    failed_list.append(
                        {"id": item.id, "file_path": item.file_path, "reason": reason}
                    )
                ...
                await self._quarantine_candidate(
                    candidate, actual_path, quarantine_root
                )
                ...
                failed_list.append(
                    {"id": item.id, "file_path": item.file_path, "reason": str(e)}
                )
        ...
        if audit_service and success_count > 0:
            try:
                await audit_service.log_operation(
                    ...
                    if not failed_list
                    else AuditOperationResult.PARTIAL,
                    error_message=f"失败 {len(failed_list)} 个"
                    if failed_list
                    else None,
                )
            except Exception as e:
    ...
    async def auto_cleanup_expired(
        ...
        manifest = await self._build_realtime_manifest(
            store, {candidate.downloader_id for candidate in purgeable}
        )
        if manifest is None:
            return {
                "quarantined_count": 0,
        ...
        logger.info(
            f"[孤儿自动清理] 发现 {len(purgeable)} 个满足条件的候选，移入隔离区"
        )
        ...
        for candidate in purgeable:
            try:
                if normalize_path(candidate.canonical_path) in manifest.expected_paths:
                    logger.warning(
                        f"[孤儿自动清理] 文件已被种子引用，跳过: {candidate.canonical_path}"
                    )
                    failed_count += 1
                    continue
                if not self._path_authorized(
                    candidate, manifest
                ) or not self._identity_complete(candidate):
                    logger.warning(
                        f"[孤儿自动清理] 路径未授权或身份字段不完整: {candidate.canonical_path}"
                    )
                    failed_count += 1
                    continue
                ...
                await _lease_handle.assert_owned()
                await self._quarantine_candidate(
                    candidate, candidate.canonical_path, quarantine_root
                )
    ...
    async def purge_expired_quarantine(
        ...
        manifest = await self._build_realtime_manifest(
            store, {candidate.downloader_id for candidate in candidates}
        )
        if manifest is None:
            return {
                "purged_count": 0,
        ...
        for candidate in candidates:
            try:
                current_manifest = await self._build_realtime_manifest(
                    store, {candidate.downloader_id}
                )
                ...
                if (
                    ...
                    or normalize_path(candidate.canonical_path)
                ...
                if not quarantine_root:
                    ...
                    continue
                if os.path.commonpath(
                    [os.path.realpath(qpath), os.path.realpath(quarantine_root)]
                ...
                candidate.operation_state = "purge_pending"
                candidate.operation_target_path = tombstone_path
                ...
                await _lease_handle.assert_owned()
                delete_manifest = await self._build_realtime_manifest(
                    store, {candidate.downloader_id}
                )
    ...
    async def _quarantine_candidate(
        self, candidate: OrphanCurrentCandidate, source_path: str, quarantine_root: str
    ) -> str:
        ...
        await OrphanLifecycleService(self.db).mark_quarantined(
    ...
    async def _recover_interrupted_operations(
        self, manifest: ManifestSnapshot, store: Any = None, lease_handle: Any = None
    ) -> Dict[str, int]:
        """恢复上次进程在 rename/remove 与最终 DB 提交之间中断的操作。"""
        result = await self.db.execute(
            select(OrphanCurrentCandidate).where(
                OrphanCurrentCandidate.operation_state.in_(
                    ["quarantine_pending", "purge_pending"]
                )
            )
        )
        candidates = result.scalars().all()
        ...
        for candidate in candidates:
            try:
                if candidate.operation_state == "purge_pending":
                    ...
                    if target_exists:
                        refreshed = await self._build_realtime_manifest(store, {candidate.downloader_id})
                        ...
                        os.remove(target)
                        await lifecycle.mark_purged(candidate.canonical_path)
                ...
                if target and os.path.exists(target):
                    ...
                    await lifecycle.mark_quarantined(
                        candidate.canonical_path,
                        target,
                        candidate.purge_after or compute_purge_after(datetime.utcnow()),
                ...
                candidate.operation_error = str(exc)[:1000]
                await self.db.commit()
                failed += 1
                logger.error(f"[孤儿操作恢复] {candidate.canonical_path}: {exc}")
    ...
    async def trigger_scan(
        self, scan_type: str, operator: str, app: Any = None
    ) -> Dict[str, Any]:
        """触发扫描（手动/定时）"""
