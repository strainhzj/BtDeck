...
class SettingTemplateVO(BaseModel):
    ...
    id: Optional[int] = Field(default=None, description="模板ID", examples=[1])
    name: Optional[str] = Field(default=None, description="模板名称", examples=["qBittorrent标准模板"])
    description: Optional[str] = Field(
        default=None, description="模板描述", examples=["适用于qBittorrent的标准配置模板"]
    )
    ...
    template_config: Optional[dict] = Field(default=None, alias="templateConfig", description="模板配置(JSON对象)")
    is_system_default: Optional[bool] = Field(
        None, alias="isSystemDefault", description="是否系统默认模板", examples=[False]
    )
    created_by: Optional[int] = Field(default=None, alias="createdBy", description="创建者用户ID")
    ...
    path_mapping: Optional[dict] = Field(default=None, alias="pathMapping", description="路径映射配置")
