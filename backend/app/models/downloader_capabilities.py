...
class DownloaderCapabilities(Base):
    ...
    supports_transfer_speed = Column(Boolean, nullable=False, server_default="1", comment="是否支持传输速度控制")
    supports_connection_limits = Column(Boolean, nullable=False, server_default="1", comment="是否支持连接限制")
    supports_queue_settings = Column(Boolean, nullable=False, server_default="1", comment="是否支持队列设置")
    supports_download_paths = Column(
        Boolean, nullable=False, server_default="0", comment="是否支持路径设置（Transmission支持）"
    )
    supports_port_settings = Column(Boolean, nullable=False, server_default="1", comment="是否支持端口设置")
    supports_advanced_settings = Column(Boolean, nullable=False, server_default="1", comment="是否支持高级设置")
    supports_peer_limits = Column(Boolean, nullable=False, server_default="0", comment="是否支持Peer限制")

    # ========== 扩展能力配置（JSON格式） ==========
    extended_capabilities = Column(Text, nullable=True, comment="扩展能力配置（JSON格式），用于存储未来新增的能力开关")

    # ========== 同步状态 ==========
    synced_from_downloader = Column(Boolean, nullable=False, server_default="0", comment="是否已从下载器同步过能力")
