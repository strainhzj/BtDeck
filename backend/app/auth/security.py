...
# bcrypt 完整哈希固定长度（版本 2 字符 + $ + cost 2 字符 + $ + 盐+校验和 53 字符）
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    if is_bcrypt_hash(hashed_password):
        ...
        # 新版 bcrypt（Rust 实现）对前缀正确但截断的哈希直接 panic 而非抛
        # ValueError；先做结构校验保证纯布尔返回契约不被破坏。
        if len(stored) != _BCRYPT_HASH_LENGTH:
            logger.warning(f"bcrypt 哈希长度异常（{len(stored)}），按验证失败处理")
            return False
        try:
