...
class TorrentFileBackupCreate(BaseModel):
    ...
    source_file_path: Optional[str] = Field(
        None, max_length=1000, description="源文件路径（如果从路径备份）", examples=["/path/to/torrent/file.torrent"]
    )
...
class TorrentFileBackupListResponse(BaseModel):
    ...
    list: List[TorrentFileBackupResponse] = Field(default_factory=list, description="种子文件备份列表")
...
class TorrentFileBackupBatchCreate(BaseModel):
    ...
    backup_requests: List[TorrentFileBackupCreate] = Field(
        ..., min_length=1, max_length=50, description="备份请求列表（最多50个）"
    )
...
class TorrentFileBackupBatchResponse(BaseModel):
    ...
    success_items: List[TorrentFileBackupResponse] = Field(default_factory=list, description="成功的备份项")
    failed_items: List[dict] = Field(default_factory=list, description="失败的备份项（包含info_hash和错误信息）")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "failed_count": 2,
                "success_items": [],
                "failed_items": [{"info_hash": "abc123...", "error": "文件不存在"}],
            }
        }
