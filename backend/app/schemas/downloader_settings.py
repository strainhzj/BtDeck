...
class DownloaderSettingBase(BaseModel):
    ...
    dl_speed_limit: int = Field(
        ..., ge=0, description="全局下载速度限制，数值含义取决于 dl_speed_unit，0表示不限速", alias="dlSpeedLimit"
    )
    ul_speed_limit: int = Field(
        ..., ge=0, description="全局上传速度限制，数值含义取决于 ul_speed_unit，0表示不限速", alias="ulSpeedLimit"
    )
    dl_speed_unit: SpeedUnitEnum = Field(
        default=SpeedUnitEnum.KB_PER_SEC, description="下载速度单位：0=KB/s, 1=MB/s", alias="dlSpeedUnit"
    )
    ul_speed_unit: SpeedUnitEnum = Field(
...
class SpeedScheduleRuleBase(BaseModel):
    ...
    start_time: str = Field(
        ..., pattern=r"^\d{2}:\d{2}:\d{2}$", description="开始时间，格式：HH:MM:SS", alias="startTime"
    )
    ...
    days_of_week: str = Field(
        ..., min_length=1, max_length=7, description="生效星期几，0=周日，1=周一，...，6=周六", alias="daysOfWeek"
    )
...
class DownloaderSettingWithRules(DownloaderSettingResponse):
    ...
    speed_schedule_rules: List[SpeedScheduleRuleResponse] = Field(
        default=[], description="分时段速度规则列表", alias="speedScheduleRules"
    )
