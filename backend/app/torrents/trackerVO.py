...
class TrackerInfoVO(BaseModel):
    ...
    tracker_id: Optional[str] = Field(default=None, description="tracker数据主键", examples=["t1"])
    tracker_name: Optional[str] = Field(default=None, description="tracker名称字符串", examples=["tracker.example.com"])
    tracker_url: Optional[str] = Field(
        None, description="tracker地址字符串", examples=["http://tracker.example.com:2710/announce"]
    )
    last_announce_succeeded: Optional[str] = Field(default=None, description="announce状态字符串", examples=["1"])
