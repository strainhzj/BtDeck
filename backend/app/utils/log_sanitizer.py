...
# 用户名常见模式（邮箱、简单用户名）
USERNAME_PATTERN = re.compile(r'(username|user|account)[=:\s]+([^\s,\'"}]+)', re.IGNORECASE)
...
def sanitize_log_message(message: str) -> str:
    ...
    # 脱敏IP地址
    sanitized = IP_PATTERN.sub(lambda m: sanitize_ip(m.group()), message)

    # 脱敏用户名
    sanitized = USERNAME_PATTERN.sub(lambda m: f"{m.group(1)}=***", sanitized)

    return sanitized


def format_connection_log(nickname: str, host: str, port: Any, sanitize: bool = True) -> str:
    """格式化连接日志（可选脱敏）
