...
| **F811（重复 import 部分）** | 15 | 5 | 🔶 清了 10 处（7 模块级重复 import 删除 + 3 函数内局部 import noqa），剩 5 处是高风险项见下 |
...
| E711/E712 | 与 None/False 比较 | 47 | 需甄别 ORM 查询（高风险，见遗留项） |
...
5. **P4**：E711/E712（E722 已清，剩 47 处 None/False 比较，需区分 Python 比较与 ORM 查询）
...
8. **长期**：mypy 类型标注补全（app/models/ 真实 bug 已清，剩余 ORM 债待 SQLAlchemy 2.0 迁移）

## 高风险遗留项（需单独处理，不混入常规 lint 清理）
