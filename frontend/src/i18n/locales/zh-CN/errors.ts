...
export const errors = {
  ...
  byCode: {
    ...
    rssFeedListFailed: '获取订阅源列表失败，请稍后重试',
    rssFeedCreateFailed: '创建订阅源失败，请稍后重试',
    rssFeedUpdateFailed: '更新订阅源失败，请稍后重试',
    rssFeedDeleteFailed: '删除订阅源失败，请稍后重试',
    rssFeedRefreshFailed: '订阅源刷新失败，请稍后重试',
    rssFeedNotFound: '订阅源不存在',
    rssFeedNameInvalid: '订阅源名称不能为空且不超过200字符',
    rssFeedUrlInvalid: '订阅源地址必须是合法的 http/https 链接',
    rssFeedDuplicate: '该下载器下已存在相同地址的订阅源',
    rssFeedFetchFailed: '订阅源抓取失败，请检查地址与网络',
    rssFeedParseFailed: '订阅源解析失败，内容不是有效的 RSS/Atom',
    rssArticleListFailed: '获取文章列表失败，请稍后重试',
    rssArticleNotFound: '文章不存在',
    rssArticleStatusInvalid: '无效的文章状态筛选',
    rssArticleAlreadyAdded: '文章已推送过，不可重复推送',
    rssArticleLinkUnsupported: '文章链接不是可推送的 magnet 或直链地址',
    rssDownloaderNotFound: '目标下载器不存在或未连接',
    rssDownloaderOffline: '目标下载器当前不可用',
    rssDownloaderConnectionMissing: '目标下载器客户端连接不存在',
    rssDownloaderTypeUnsupported: '该下载器类型暂不支持 RSS 推送',
    rssAddFailed: '推送到下载器失败，请稍后重试',
    rssAddStateFailed: '推送成功但状态记录失败，请勿重复推送',
    rssStoreUnavailable: '下载器缓存未初始化',
    rssPlatformUnsupported: '当前运行形态不支持 RSS 订阅功能',
    rssFeedIntervalInvalid: '刷新间隔必须在 5-1440 分钟之间',
    // ── RSS Phase 2（feature rss-subscription-phase2-2026-09-24）：模式/规则/qB 代理 ──
    rssModeInvalid: '无效的 RSS 模式（可选 BtDeck 引擎 / qB 原生）',
    rssModeTypeUnsupported: '仅 qBittorrent 下载器支持 qB 原生 RSS 模式',
    rssModeCapabilityDisabled: '该下载器的 RSS 管理能力已被关闭',
    rssModeUpdateFailed: '切换 RSS 模式失败，请稍后重试',
    rssModeGetFailed: '获取 RSS 模式失败',
    rssModeConflict: '目标下载器已切换为 qB 原生 RSS 模式，请在该下载器内管理订阅',
    rssRuleInvalid: '规则参数无效',
    rssRuleNameInvalid: '规则名称不能为空且不超过200字符',
    rssRuleKeywordsInvalid: '命中关键词不能为空（逗号分隔，至少一个）',
    rssRuleRegexInvalid: '非法正则表达式',
    rssRuleTargetInvalid: '目标下载器不存在或类型不支持',
    rssRuleSavePathInvalid: '保存路径不能超过500字符',
    rssRuleTagsInvalid: '标签不能超过200字符',
    rssRuleFeedInvalid: '关联订阅源不存在或不属于该下载器',
    rssRuleDuplicate: '该下载器下已存在同名规则',
    rssRuleNotFound: '规则不存在',
    rssRuleCreateFailed: '创建规则失败，请稍后重试',
    rssRuleUpdateFailed: '更新规则失败，请稍后重试',
    rssRuleDeleteFailed: '删除规则失败，请稍后重试',
    rssRuleListFailed: '获取规则列表失败，请稍后重试',
    rssRulePreviewFailed: '匹配预览失败，请稍后重试',
    rssQbTypeUnsupported: '仅 qBittorrent 下载器支持原生 RSS 管理',
    rssQbProxyFailed: 'qBittorrent RSS 操作失败，请稍后重试',
    rssQbFeedPathInvalid: 'RSS 项目路径无效',
    rssQbFeedUrlInvalid: '订阅源地址必须是合法的 http/https 链接',
    rssQbFeedNotFound: 'qB 订阅源路径不存在',
    rssQbRuleNameInvalid: '规则名称不能为空且不超过200字符',
    rssQbRuleDefInvalid: '规则定义字段类型无效',
    rssQbPrefValueInvalid: '偏好取值超出允许范围',
    rssQbPrefKeyRejected: '没有可更新的偏好字段（仅支持白名单四键）',
