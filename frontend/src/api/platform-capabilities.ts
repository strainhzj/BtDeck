...
/**
 * 能力缓存必须是响应式的：能力门控 computed（设置弹窗 pathMappingAvailable、
 * 移动抽屉 mobileMenuItems 等）读取模块级缓存是非响应式依赖——首次求值（缓存
 * 未加载时 fail-closed 为 false/unknown）后被 Vue 永久缓存，$forceUpdate 也不
 * 会重算，缓存后到也救不回（demo 快速路径必现；真实环境慢加载同样可能中招）。
 * 用 Vue.observable 包一层，缓存写入自动触发所有依赖它的 computed 重算。
 */
const reactiveCache = Vue.observable<{ data: PlatformCapabilitiesData | null }>({ data: null })
...
export function loadPlatformCapabilities(force = false): Promise<PlatformCapabilitiesData | null> {
  if (reactiveCache.data && !force) return Promise.resolve(reactiveCache.data)
  ...
  inflight = request<ApiEnvelope<PlatformCapabilitiesData>>({
    ...
    .then(res => {
      ...
      reactiveCache.data = res?.status === 'success' ? res.data : null
      return reactiveCache.data
    })
    .catch(() => {
      reactiveCache.data = null
      return null
    })
    .finally(() => {
      inflight = null
...
export function cachedCapabilityLevel(key: string): CachedCapabilityLevel {
  const level = reactiveCache.data?.capabilities[key]?.level
...
export function cachedPlatform(): 'desktop' | 'android-server' | 'unknown' {
  return reactiveCache.data?.platform ?? 'unknown'
}
