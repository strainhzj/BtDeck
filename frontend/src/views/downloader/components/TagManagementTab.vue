<template>
  <div class="tag-management-tab">
    ...
    <div v-if="!downloader" class="empty-state">
      ...
      <h3>{{ $t('downloader.tag.needBasicTitle') }}</h3>
      <p>{{ $t('downloader.tag.needBasicDesc') }}</p>
    </div>
    ...
    <div v-else class="tag-content">
      ...
      <div class="toolbar">
        <div class="toolbar-left">
          <el-input
            ...
            :placeholder="$t('downloader.tag.searchPlaceholder')"
            style="width: 280px;"
            @input="handleSearchInput"
          >
            <template slot="prefix"><LucideIcon name="search" :size="14" /></template>
            <template slot="suffix">
              <button v-if="searchKeyword" class="input-clear" type="button" :aria-label="$t('downloader.tag.clearSearch')" @click="handleSearchClear">
      ...
      <!-- 排序选项 -->
      <div class="sort-bar">
        <span class="sort-label">{{ $t('downloader.tag.sortLabel') }}</span>
        <el-select
          v-model="sortBy"
          size="small"
          @change="loadTags"
        >
          <el-option :label="$t('downloader.tag.sortCreated')" value="created_at" />
          <el-option :label="$t('downloader.tag.sortName')" value="tag_name" />
        </el-select>
        <el-button
          size="mini"
        ...
        >
          ...
          {{ sortOrder === 'asc' ? $t('downloader.tag.sortAsc') : $t('downloader.tag.sortDesc') }}
        </el-button>
      </div>
      ...
      <div
        v-loading="loading"
        class="tag-list"
        :element-loading-text="$t('downloader.tag.loading')"
      >
        <!-- 空状态 -->
        <div v-if="filteredTags.length === 0 && !loading" class="empty-tags">
          ...
          <p>{{ searchKeyword ? $t('downloader.tag.emptyNoMatch') : $t('downloader.tag.emptyNone') }}</p>
        </div>
          ...
          >
            <div class="tag-header">
              <div class="tag-type-badge">
                ...
                <span>{{ tag.tag_type === 'category' ? $t('downloader.tag.typeCategory') : $t('downloader.tag.typeTag') }}</span>
              </div>
            ...
            <div class="tag-footer">
              <span class="tag-time">{{ $t('downloader.tag.createdAt', {time: formatTime(tag.created_at)}) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item :label="$t('downloader.tag.dialogName')" prop="tag_name">
          <el-input
            v-model="tagForm.tag_name"
            :placeholder="$t('downloader.tag.namePlaceholder')"
            maxlength="255"
            show-word-limit
          />
        </el-form-item>
        <el-form-item :label="$t('downloader.tag.dialogType')" prop="tag_type">
          <el-radio-group v-model="tagForm.tag_type">
            <el-radio label="category">{{ $t('downloader.tag.typeCategory') }}</el-radio>
            <el-radio label="tag">{{ $t('downloader.tag.typeTag') }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item :label="$t('downloader.tag.color')" prop="color">
          <el-color-picker
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="showDialog = false">{{ $t('downloader.tag.cancel') }}</el-button>
        <el-button type="primary" :disabled="submitting" @click="handleSubmit">
          ...
          {{ editingTag ? $t('downloader.tag.save') : $t('downloader.tag.create') }}
        </el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('downloader.tag.batchDeleteTitle')"
    ...
    >
      <div class="batch-delete-content">
        ...
        <p>{{ $t('downloader.tag.batchDeletePrefix') }} <strong>{{ selectedTags.length }}</strong> {{ $t('downloader.tag.batchDeleteSuffix') }}</p>
        <p class="warning-text">{{ $t('downloader.tag.batchDeleteWarning') }}</p>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="showBatchDeleteDialog = false">{{ $t('downloader.tag.cancel') }}</el-button>
        <el-button type="danger" :disabled="batchDeleting" @click="handleConfirmBatchDelete">
          ...
          {{ $t('downloader.tag.confirmDelete') }}
        </el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('downloader.tag.categoryDialogTitle')"
      width="500px" custom-class="tag-mgmt-dialog"
    ...
    >
      <div class="category-select-content">
        ...
        <p class="dialog-title">
          {{ $t('downloader.tag.categoryDialogPrefix') }}<strong>{{ deletingTag ? deletingTag.tag_name : '' }}</strong>{{ $t('downloader.tag.categoryDialogSuffix') }}
        </p>
        <p class="dialog-desc">{{ $t('downloader.tag.categoryDialogDesc') }}</p>
        
        <el-form label-width="80px" style="margin-top: 20px;">
          <el-form-item :label="$t('downloader.tag.targetCategory')">
            <el-select
              v-model="targetCategory"
              :placeholder="$t('downloader.tag.targetCategoryPlaceholder')"
              clearable
              style="width: 100%;"
            >
              <el-option :label="$t('downloader.tag.uncategorized')" value=""></el-option>
              <el-option
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="handleCancelDeleteCategory">{{ $t('downloader.tag.cancel') }}</el-button>
        <el-button type="primary" :disabled="transferring" @click="handleConfirmDeleteCategory">
          ...
          {{ $t('downloader.tag.moveAndDelete') }}
        </el-button>
      </div>
    </el-dialog>
  </div>
...
export default class TagManagementTab extends Vue {
  ...
  private typeOptions = [
    { label: this.$t('downloader.tag.filterAll').toString(), value: 'all' as const },
    { label: this.$t('downloader.tag.typeCategory').toString(), value: 'category' as const },
    { label: this.$t('downloader.tag.typeTag').toString(), value: 'tag' as const }
  ]
  ...
  get dialogTitle(): string {
    return this.editingTag
      ? this.$t('downloader.tag.dialogEditTitle').toString()
      : this.$t('downloader.tag.dialogAddTitle').toString()
  }
  ...
  get tagFormRules() {
    return {
      tag_name: [
        { required: true, message: this.$t('downloader.tag.validate.nameRequired').toString(), trigger: 'blur' },
        { min: 1, max: 255, message: this.$t('downloader.tag.validate.nameLength').toString(), trigger: 'blur' }
      ],
      tag_type: [
        { required: true, message: this.$t('downloader.tag.validate.typeRequired').toString(), trigger: 'change' }
      ]
    }
  }
  ...
  private handleDelete(tag: TorrentTag) {
    ...
    if (tag.tag_type === 'category') {
      ...
      // 标签直接删除确认
      MessageBox.confirm(
        this.$t('downloader.tag.msg.deleteConfirm', { name: tag.tag_name }).toString(),
        this.$t('downloader.tag.msg.deleteTitle').toString(),
        {
          confirmButtonText: this.$t('downloader.tag.msg.confirm').toString(),

          cancelButtonText: this.$t('downloader.tag.msg.cancel').toString(),
          type: 'warning'
        }
      ).then(async() => {
        try {
          await deleteTag(tag.tag_id)
          Message.success(this.$t('downloader.tag.msg.deleteSuccess').toString())
          await this.loadTags()
        } catch (error) {
  ...
  private handleBatchDelete() {
    if (this.selectedTags.length === 0) {
      Message.warning(this.$t('downloader.tag.msg.selectFirst').toString())
      return
    }
  ...
  private async handleConfirmBatchDelete() {
    ...
    try {
      ...
      Message.success(this.$t('downloader.tag.msg.batchDeleted', { count: tagIds.length }).toString())
  ...
  private async handleSubmit() {
    ...
    try {
      ...
      if (this.editingTag) {
        ...
        Message.success(this.$t('downloader.tag.msg.updateSuccess').toString())
      } else {
        // 创建模式
        await createTag(this.tagForm as CreateTagRequest)
        Message.success(this.$t('downloader.tag.msg.createSuccess').toString())
      }
  ...
  private formatTime(timeStr: string): string {
    ...
    if (days === 0) return translate('downloader.tag.time.today')
    if (days === 1) return translate('downloader.tag.time.yesterday')
    if (days < 7) return translate('downloader.tag.time.daysAgo', { n: days })
    if (days < 30) return translate('downloader.tag.time.weeksAgo', { n: Math.floor(days / 7) })
    if (days < 365) return translate('downloader.tag.time.monthsAgo', { n: Math.floor(days / 30) })
    return translate('downloader.tag.time.yearsAgo', { n: Math.floor(days / 365) })
  }
